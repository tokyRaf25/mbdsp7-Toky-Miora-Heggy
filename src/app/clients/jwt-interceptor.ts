...
export class JwtInterceptor implements HttpInterceptor{
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        //const url = 'https://back-assignments-mbds1055.herokuapp.com/';
         const url = 'https://apinodeapp.herokuapp.com/';
        console.log(url);
        ...
        if (currentClients && currentClients.token) {
            console.log(`Bearer ${currentClients.token}`);
            request = request.clone({
                setHeaders: {
                    'Authorization': `Bearer ${currentClients.token}`
                }
            });
        }
